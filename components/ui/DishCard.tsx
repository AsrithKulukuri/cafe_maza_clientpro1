...
export function DishCard({ dish, onAdd }: DishCardProps) {
    ...
    return (
        <motion.article
            ...
            className="group overflow-hidden rounded-2xl border border-[#CFAF63]/30 bg-gradient-to-br from-[#1A1816] to-[#0F0D0B] backdrop-blur-sm"
        >
            ...
            <div className="space-y-4 p-5 group-hover:shadow-[0_0_60px_rgba(255,106,0,0.25)] transition-shadow duration-500">
                <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                        <h3 className="font-[var(--font-heading)] text-2xl text-[#F5F5F5] leading-tight">{dish.name}</h3>
                        <motion.span
                    ...
                    <span className="whitespace-nowrap text-xl font-[var(--font-heading)] text-[#CFAF63]">₹{dish.price}</span>
                </div>
                ...
                {onAdd ? (
                    <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        className="w-full rounded-lg border border-[#FF6A00]/50 bg-gradient-to-r from-[#FF6A00]/20 to-[#CFAF63]/20 px-4 py-3 text-sm font-semibold text-[#F5F5F5] transition hover:border-[#FF6A00] hover:from-[#FF6A00]/35 hover:to-[#CFAF63]/35"
                    >
                        + Add to Cart
                    </motion.button>
                ) : null}
